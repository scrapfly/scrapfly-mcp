...
func (p *ScrapflyToolProvider) ToolSet() tools.HandledToolSet {
	// Only the always-visible "static" tools are returned here. The
	// MCP runtime sends these in tools/list right after handshake.
	// Browser-interaction tools (click, fill, take_snapshot, ...)
	// are NOT in this set — they get registered dynamically when
	// cloud_browser_open / browser_unblock succeed (see
	// mountInteractionTools below) and removed on close.
	return staticTools(p)
}

// dynamicInteractionTools is the catalog of tools that only make
// sense when a Cloud Browser session is open. They are mounted at
// run-time by mountInteractionTools (after cloud_browser_open /
// browser_unblock) and unmounted by unmountInteractionTools (on
// cloud_browser_close or session expiry). Each call to this builder
// returns a FRESH HandledToolSet — handlers close over the provider,
// not over a specific session, so they look up the active session
// via browser.FindSession("") at call time.
func (p *ScrapflyToolProvider) dynamicInteractionTools() tools.HandledToolSet {
	return interactionTools(p)
}

// mountInteractionTools registers the per-session interaction tools
// onto the live *mcp.Server. Idempotent — safe to call after every
// cloud_browser_open even if a session was already open. The MCP
// server's AddTool replaces existing tools with the same name, and
// `notifications/tools/list_changed` fires automatically so connected
// clients refetch.
func (p *ScrapflyToolProvider) mountInteractionTools() {
	if p.MCPServer == nil {
		// During tests or static toolset enumeration there's no live
		// server. The tools simply won't be exposed; the open call
		// still succeeds.
		return
	}
	set := p.dynamicInteractionTools()
	for _, ht := range set {
		p.MCPServer.AddTool(ht.Tool, ht.Handler)
	}
	p.logger.Printf("mounted %d interaction tools on browser open", len(set))
}

// unmountInteractionTools removes the per-session interaction tools
// from the live *mcp.Server. Safe to call when no session is open
// (RemoveTools is a no-op for unknown names).
func (p *ScrapflyToolProvider) unmountInteractionTools() {
	if p.MCPServer == nil {
		return
	}
	names := p.dynamicInteractionTools().Names()
	p.MCPServer.RemoveTools(names...)
	p.logger.Printf("unmounted %d interaction tools on browser close", len(names))
}
...
// staticTools — the always-visible tool surface. Returned by
// ToolSet() and registered at MCP server boot via tools/list. These
// don't require a Cloud Browser session: account info, stateless
// scraping, antibot classification, the cheat-sheet prompt, and
// browser-LIFECYCLE tools (open / unblock / close / sessions).
//
// What's NOT here: every interaction tool (click, fill, take_snapshot,
// take_screenshot, scroll, evaluate_script, …) AND the cloud-browser
// helpers that operate on an active session (cloud_browser_navigate,
// cloud_browser_screenshot, cloud_browser_eval, cloud_browser_performance,
// cloud_browser_downloads, list_webmcp_tools, call_webmcp_tool). Those
// are added dynamically when cloud_browser_open / browser_unblock
// succeeds — see interactionTools below.
func staticTools(provider *ScrapflyToolProvider) tools.HandledToolSet {
	HandledTools := tools.NewHandledToolset()
	tools.MustAddToolToToolset(HandledTools, &mcp.Tool{ // alias
		Name:        "info_account",
	...
	// WebMCP meta-tools (list_webmcp_tools / call_webmcp_tool) are
	// always reachable. They no-op when no browser session is open
	// AND let the model reach page-registered tools across the
	...
	// part of the always-visible static surface. We tried registering
	// them dynamically only after cloud_browser_open succeeds, but
	// both ADK-Python (mcp_toolset) and ADK-Go (mcptoolset) resolve
	// tools ONCE per LLM exchange — which means a multi-step turn
	// like "open → click" can't dispatch the click because the
	// resolved tool list is from before the open. Each interaction
	// tool's handler errors gracefully ("no active browser session")
	// when called without a session, so always-on registration is
	// safe. Dynamic mount/unmount still happens via mountInteractionTools
	// for clients that DO refetch on notifications/tools/list_changed.
	for name, ht := range interactionTools(provider) {
...
// interactionTools — the dynamic, browser-session-only tool surface.
// Mounted onto the *mcp.Server when cloud_browser_open / browser_unblock
// succeed, unmounted on cloud_browser_close. Each handler still does a
// browser.FindSession("") guard so a tool call that races with an
// unmount returns a clean error rather than panicking.
//
// Includes: cloud_browser_navigate / _screenshot / _eval / _performance
// / _downloads (the older "cloud_browser_*" surface), the flat
// interaction tools (click / fill / type_text / hover / press_key /
// scroll / select_option / drag / take_snapshot / take_screenshot /
// get_page_url / evaluate_script), and the WebMCP meta-tools
// (list_webmcp_tools / call_webmcp_tool).
func interactionTools(provider *ScrapflyToolProvider) tools.HandledToolSet {
	HandledTools := tools.NewHandledToolset()
	...
	// (list_webmcp_tools, call_webmcp_tool). Defined in
	// tools_browser_static.go but exposed dynamically through this
	// function — the file name is historical.
